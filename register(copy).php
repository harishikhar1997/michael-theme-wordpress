...
function register_action{
	...
	//	print_r($_POST);die();
		...
		if(empty($username)) { 
			...
			echo json_encode(array('code'=>2, 'message'=>__('Username already exists, please try another')));
		...
		if( !is_email( $email ) ) { 
			...
			echo json_encode(array('code'=>3, 'message'=>__('Please enter a valid email')));

			$errors['email'] = "Please enter a valid email";
		} elseif( email_exists( $email ) ) {

			echo json_encode(array('code'=>4, 'message'=>__('This email address is already in use')));
		...
		if(0 === preg_match("/.{6,}/", $_POST['password'])){
		  ...
		  echo json_encode(array('code'=>5, 'message'=>__('TPassword must be at least six characters')));
